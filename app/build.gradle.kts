...
fun getOutputZip(): File? {
    val zipFile = File("${project(":js").buildDir}/distributions/${OutputJarName}.zip")
    if (!zipFile.exists()) {
        logger.lifecycle("Zip cannot be found at ${zipFile.path}")
        return null
    }
    return zipFile
}
...
tasks {
    register("copyJar") {
        ...
        doLast {
            ...
            if (org.gradle.internal.os.OperatingSystem.current().isWindows) {
                ...
                File(modsFolder, "$OutputJarName.zip").delete()
                File(modsFolder, "${OutputJarName}Desktop.jar").delete()
                val outputZip = getOutputZip()
                if (outputZip != null)
                    copyJarFile(outputZip, modsFolder)
    ...
    register("downloadDesktop") {
        ...
        doLast {
            create<DownloadTask>("${name}Wrapper") {
                sourceUrl.set("http://github.com/Anuken/Mindustry/releases/download/n${GameLaunchVersion}/Mindustry.jar")
    ...
    register("downloadServer") {
        ...
        doLast {
            create<DownloadTask>("${name}Wrapper") {
                sourceUrl.set("https://github.com/Anuken/Mindustry/releases/download/n${GameLaunchVersion}/server-release.jar")
